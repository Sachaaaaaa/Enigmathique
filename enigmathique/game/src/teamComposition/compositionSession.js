...
class CompositionSession {
  ...
  refreshAvailableStudents = () => {
    ...
    this.availableStudents = this.students.filter((s) => !this.teamSockets.some((t) => t.hasStudent(s.id)));
  };
  ...
  sendAvailableToTeams = () => {
    this.teamSockets.forEach((t) => t.sendAvailableStudents(this.availableStudents));
  };
  ...
  sendCompositionToProfessor = () => {
    ...
    this.professorSockets.forEach((p) => p.sendComposition(this.availableStudents, lockedTeams, confirmedTeams));
  };
  ...
  finishComposition = async () => {
    ...
    // Supprime les équipes qui ne sont pas confirmées, leur envoie un message de refus
    const refusedTeams = this.teamSockets.filter((t) => !t.confirmed);
    refusedTeams.forEach((t) => {
      t.sendError("La partie est lancée, votre équpe n'a pas été confirmée", true);
      t.socket.disconnect();
      // Supprime la socket de la liste
      this.onTeamLeave(t);
    });
