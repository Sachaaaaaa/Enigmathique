...
class GameSession {
	...
	areAllTeamsReady = () => {
		return (
			this.getTotalTeamCount() > 0 && this.teams.every((team) => team.isReady)
		);
	};
	...
	addTeam = (team) => {
		...
		if (this.isSessionRunning) {
			console.log(
				clc.redBright('[Session] Equipe refusée, la session est déjà lancée')
			);
			return;
		}
		...
		if (this.teams.some((t) => t.teamId == team.teamId)) {
			console.log(
				clc.redBright(
					`[Session] L\'équipe ${team.teamId} est déjà dans la session`
				)
			);
			return;
		}
		...
		if (!this.expectedTeams.some((t) => t == team.teamId)) {
			console.log(
				clc.redBright(`[Session] L\'équipe ${team.teamId} n'est pas attendue`)
			);
			return;
		}
		...
		if (this.expectedTeams.length == this.teams.length) {
			console.log(
				clc.greenBright('[Session] Toutes les équipes sont présentes')
			);
			this.startSession();
		}
	};
	...
	broadcastStartRound = () => {
		...
		console.log(
			clc.greenBright(
				`[Session] Lancement du timer (${TIME_PER_ROUND} secondes)`
			)
		);
	...
	tick = () => {
		if (this.isPlaying) {
			...
			console.log(
				clc.greenBright(`[Session] Tick: ${timeLeft} secondes restantes`)
			);
			...
			console.log(
				clc.greenBright(
					`[Session] Tick: En attente de chargements des joueurs ...`
				)
			);
		} else {
			console.log(
				clc.greenBright(
					`[Session] Tick: En attente de joueurs (${this.getTotalTeamCount()} / ${
						this.expectedTeams.length
					})...`
				)
			);
		}
	...
	onTeamLoadedRoom = (team) => {
		...
		const isAllRoomsLoaded = this.teams.every(
			(team) => team.leaved || team.haveLoadedRoom
		);

		if (isAllRoomsLoaded) {
			console.log(
				clc.green('[Session] Toutes les équipes ont chargé leur salle')
			);
	...
	onTeamSolvedRoom = (team) => {
		...
		const doesEveryoneSolved = this.teams.every(
			(team) =>
				team.leaved ||
				team.currentRoom.enigmasSolved.length == team.currentRoom.enigmas.length
		);

		if (doesEveryoneSolved) {
			console.log(
				clc.green('[Session] Toutes les équipes ont résolu leur salle')
			);
			this.rotateRooms();
		}
